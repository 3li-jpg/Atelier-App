...
- **Open:** (a) supervisor-side: emit `question` events + deliver `user_message` replies to the harness. **Design resolved (openchamber research):** switch the supervisor from `opencode run` (one-shot, no reply channel) to `opencode serve` (long-lived HTTP API on 127.0.0.1). Drive it: `POST /session/{id}/prompt_async` to start, subscribe to `GET /event` SSE, forward `permission.updated`/question requests to the API as `question` events, relay user replies via `POST /question/{requestID}/reply` (or `/session/{id}/permissions/{id}` for approvals). Auth via `OPENCODE_SERVER_PASSWORD`. — **implementation pending in `runner/`**; (b) supervisor idle heartbeat for model-wait >30 s detection; ~~(c) reaper orphan scan~~ ✅ done — `SandboxProvider.listMachines()` + `FlyMachinesProvider.listMachines()` (GET /v1/apps/:app/machines → `MachineInfo`); `orchestrator.reapOrphans()` runs in every `sweep()`, destroys machines tagged `atelier_session` whose session is terminal/missing (foreign + already-destroyed machines left alone); tested in `api.test.ts` + `fly.test.ts`; (d) chaos test: kill API mid-session, restart, verify cleanup. Note the in-memory timers die with the process — the reaper covers TTLs after restart, but a suspended machine's stop-demotion timer is lost until its session is swept (the orphan scan now catches the leaked machine on the next sweep).
