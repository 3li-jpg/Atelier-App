...
- **Audit fixes (2026-07-06):** supervisor now honors PRD §9.3 — opencode config lives on tmpfs (`/dev/shm`, never the persistent rootfs); `LLM_API_KEY` + `GIT_TOKEN` are scrubbed from PID 1's env after use (git token stashed in a tmpfs cred file, origin URL stripped of the token, push uses `http.extraHeader`). `.dockerignore` now excludes `.env*` so secrets aren't baked into the API image. Runner image needs a rebuild to pick these up.
...
- **Audit fixes (2026-07-06, amended):** `max_turns` now enforced (counts `running` states, kills over budget); supervisor-driven terminal transitions now accrue billing (`onSupervisorState` routes through `accrueBilling`); stale suspend timer cleared on `running`. ⚠️ Three of the original audit "fixes" (stop demotion, 30 s suspend, wall-clock reaping) were **reverted** — they followed PRD D6's batch model and broke interactive workspaces (handoff-openchamber.md Task 4 deliberately removed them: stop loses the rootfs/workspace, the 30 s fuse is shorter than the proxy's 60 s activity-ping throttle, and wall-clock reaping kills actively-used sessions). Where PRD D6 and handoff-openchamber.md disagree, the openchamber plan wins. Current policy: suspend after 300 s in `awaiting_user`, never stop-demote, reap on idle (`last_activity`) with a 24 h absolute cap.
...
Live at **https://atelier-control-plane.fly.dev** ("atelier-api" was taken on Fly). One deploy: Hono serves the built web bundle (`WEB_DIST`) + the API; `apps/api/Dockerfile`, sqlite on the `atelier_data` volume at `/data`. Deploy with `flyctl deploy -c infra/fly.api.toml --ha=false` from the repo root. Secrets set: MASTER_KEY, AUTH_TOKEN, SESSION_SECRET, FLY_SANDBOX_TOKEN (deploy token scoped to atelier-sandboxes), FLY_SANDBOX_APP, PUBLIC_URL,
**DB (2026-07-06):** `apps/api/src/pg-store.ts` — Postgres backend (Supabase in prod, plain `postgres` driver, no supabase-js) selected by `DATABASE_URL`; sqlite stays the zero-config default. Store surface is awaited everywhere; timestamps stored as sqlite-format UTC text so `new Date(x+"Z")` consumers are unchanged. Integration test `pg-store.test.ts` runs when `TEST_DATABASE_URL` is set (verified against local Postgres 16). 🔑 To flip prod: create a Supabase project → copy the **transaction-pooler** URI (port 6543) → `fly secrets set -a atelier-control-plane DATABASE_URL=...` → redeploy; re-add the provider in the UI (alpha data not migrated — it's one provider row). ponytail ceilings: SSE fanout is still the in-process `bus` and event seq allocation assumes one API instance — add pg LISTEN/NOTIFY + seq-conflict retry before scaling machines >1. RUNNER_IMAGE=registry.fly.io/atelier-sandboxes:runner-v7 — local copies in gitignored `.env.fly`. v3 added models.dev to the egress allowlist (opencode's catalog fetch stalls the agent without it); v6 added openchamber; v7 (2026-07-06) bakes in the audit fixes (tmpfs opencode config, env scrubbing, tokenless origin URL, bridge free-form reply injection). **Open:** GITHUB_OAUTH_CLIENT_ID/SECRET (owner must register the OAuth app, callback `https://atelier-control-plane.fly.dev/auth/github/callback`). >1 machine = swap sqlite → Postgres (don't before then).

**Local-dev note (2026-07-06):** `npm run dev -w @atelier/api` now auto-loads `.env` / `.env.fly` via Node's `--env-file-if-exists` — running the API bare left `FLY_SANDBOX_TOKEN` empty and every session failed at provisioning with `fly 401 token validation error`. The API also warns at startup if the token is missing. Hub theme now mirrors openchamber 1.14.0's default tokens (warm dark + amber, oklch, in `styles.css`; manifest/icon updated to match) so hub ↔ workspace read as one product.
